...
@section('cabecera')
    <div class="container mt-sm-3 mt-md-3 mt-lg-5 mt-xl-5">
        <h2 style="color: black;">Modificar Imagen <small style="font-size:18px;" class="text-muted">&nbsp; Última modificación {{ $imagen->updated_at->format('d-m-Y') }}.</small></h2>
...
@section('contenido')
    <div class="container mt-2">
        ...
        @endif
        <form autocomplete="off" id="form-general" method="POST" action="{{url('maquinas/'.$maquina->id.'/galeria/'.$imagen->id)}}" enctype="multipart/form-data">
            @method('PUT')
            @csrf            
            <div class="form-group">
                <label for="descripcion" class="ml-1 {{ ($errors->has('descripcion')) ? 'text-danger' : '' }}">Descripción de la Imagen</label>
                <textarea class="form-control {{ ($errors->has('descripcion')) ? 'border border-danger' : '' }}" id="descripcion" name="descripcion" rows="3">{{ (old('descripcion') == null) ? $imagen->descripcion : old('descripcion') }}</textarea>
            ...
            <div class="form-group text-center" id="imagenActual">
                <a class="example-image-link" href="{{ asset('storage/imagenes/galeria/' . $imagen->imagen) }}" data-lightbox="example-set" data-title="Imagen actual para {{ $maquina->nombre_maquina }}"><img class="example-image mt-3 ml-1 mr-1" style="border-radius: 2%;" widht="100%;" height="250px;" src="{{ asset('storage/imagenes/galeria/' . $imagen->imagen) }}"/></a>
                <br/>
