...
@section('contenido')
    <div class="container mt-3">    
        <form autocomplete="off" id="form-general" method="POST" action="{{url('/instrucciones/tipo/' . $tipo->id)}}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="nombre" class="ml-1 {{ ($errors->has('nombre')) ? 'text-danger' : '' }}">Nombre de Instrucción</label>
                <input type="text" autocomplete="off" class="form-control {{ ($errors->has('nombre')) ? 'border border-danger' : '' }}" id="nombre" name="nombre" value="{{ (old('nombre') != null) ? old('nombre') : $tipo->nombre }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ ($errors->has('descripcion_tipo')) ? 'border border-danger' : '' }}" id="descripcion" name="descripcion_tipo" rows="3">{{ (old('descripcion_tipo') != null) ? old('descripcion_tipo') : $tipo->descripcion_tipo }}</textarea>
