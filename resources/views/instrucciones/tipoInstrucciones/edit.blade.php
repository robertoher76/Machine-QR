...
@section('contenido')
    <div class="container mt-3">    
        <form autocomplete="off" id="form-general" method="POST" action="{{ route('tipo.update', $tipo) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="nombre" class="ml-1 @error('nombre') text-danger @enderror">Nombre del Tipo de Instrucción</label>
                <input type="text" autocomplete="off" class="form-control @error('nombre') is-invalid @enderror" id="nombre" name="nombre" value="{{ (old('nombre') != null) ? old('nombre') : $tipo->nombre }}">
                @error('nombre')
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('descripcion_tipo') is-invalid @enderror" id="descripcion" name="descripcion_tipo" rows="3">{{ (old('descripcion_tipo') != null) ? old('descripcion_tipo') : $tipo->descripcion_tipo }}</textarea>
        ...
        <div class="mt-3">
            <a href="{{ route('tipo.show', $tipo) }}"><i class="fas fa-chevron-left"></i>&nbsp; Regresar</a>
        </div>
    </div>
@endsection