...
@section('cabecera')
    <div class="container mt-sm-3 mt-md-3 mt-lg-5 mt-xl-5">
        <h2 class="text-body">Agregar Instrucción</h2>
        <p class="lead">Ingrese los siguientes datos para registrar una nueva instrucción a {{ $maquina->nombre_maquina }}.</p>
    </div>
@endsection
...
<div class="container mt-4">
    ...
    @endif
    <form autocomplete="off" id="form-general" method="POST" action="{{ route('maquinas.instrucciones.store', $maquina) }}">
        @csrf        
        <div class="form-group">
            <label for="titulo" class="ml-1 @error('titulo') text-danger @enderror">Título de la Instrucción</label>
            <input type="text" autocomplete="off" class="form-control @error('titulo') is-invalid @enderror" id="titulo" name="titulo" value="{{ old('titulo') }}">
            @error('titulo')
